...
- [x] **_list orders descending by the order date. The order with date 1998-05-06 should be at the top_**
  ...
  - This can be done with SELECT, FROM, and ORDER BY clauses
  </details>
...
```SQL
SELECT count(o) as "orders", c.company_name
...
```SQL
SELECT count(o) as "orders", c.contact_name
FROM orders o JOIN customers c
ON c.customer_id = o.customer_id
...
```SQL
SELECT count(o) as "orders", c.city
FROM orders o JOIN customers c
ON c.customer_id = o.customer_id
GROUP BY c.city
ORDER BY city
